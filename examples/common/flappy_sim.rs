...
/// a value-loaded confound (rising reads safe regardless of gap-relative
/// clause (\"rising\"/\"falling\") was a measured confound; v2 renders the
/// position band ALONE and excludes BOTH degenerate state classes (v =
/// FLAP_V lands both actions on one cell; same-band results render
/// identical sentences — the unbounded Below/Above bands tie most often),
/// so every committed decision has two distinct descriptions.
pub const GRAMMAR_ID: &str = "laya-flappy-v2";
...
/// The per-option sentence: `The bird {position clause}.` — the position
/// band ALONE (grammar v2; see GRAMMAR_ID for the measured v1 confound).
...
mod tests {
    ...
    fn option_sentences_always_differ_in_the_corpus() {
        // The grammar-v2 law: the enumerator excludes both degenerate
        // classes (v = FLAP_V lands both actions on one cell; same-band
        // results render identical sentences), so every committed state's
        // two options are distinct sentences — and every same-band or
        // same-cell pair is an EXCLUDED state, never a fixture row.
