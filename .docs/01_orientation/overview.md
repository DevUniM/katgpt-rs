...
| `karc_hebbian_readout` | `katgpt-core/karc_hebbian_readout` (implies `hebbian_kernel_memory`) | Hebbian KARC Readout — the `w_out` fit, Hebbian-flavored (Plan 584, riir-ai). `KarcForecaster::fit_ridge` and `HebbianKernelMemory::construct` are the SAME ridge closed form with different feature maps; this module applies the Hebbian fact-store thesis to the KARC readout: training pairs become facts (key = `pad64(delay_state)`, value = `pad64(target)` — zero-padding is inner-product-exact), and the constructed (A, G, B) plays the `w_out` role. Buys over `fit_ridge`: edit semantics (the fact-edit journal loop applies to forecasting), the per-fact margin audit, the riir-neuron-db `HebbianConstructedShard` freeze chain, and the canonical value table (observed trajectories ARE the values). f32 throughout (`fit_ridge` solves f64 — cross-arm comparisons state an f32 tolerance). **Opt-in** — GOAT ALL PASS (Bench 710): G1 five gates (pair retrieval at belief scale, bit-identical determinism, pad64 exactness, F=1 infinite margin, error shape); G2a per-fact fit ratio **0.7× — the Hebbian fit is FASTER than `fit_ridge` at runtime scale** (10.6 vs 15.7–15.9 µs/fact; the m=128 f32 closed form beats the d_h=256 f64 Cholesky while carrying the margin audit); G2b forecast wrapper Δ 0.5 ns; G4 0 allocs. `HebbianFitReport.seed` is the audit-chain single source of truth (packed into style_weights metadata by the ndb KARC-identity envelope). Private consumers: riir-neuron-db `karc_hebbian_envelope` (Plan 584 Phase 2) + riir-ai `karc_hebbian_bridge` (Phase 3 — both stay opt-in pending the swarm-homeostat production consumer; consumer-first promotion rule, cf. Issue 867 `similarity_inference` demote). See [`.benchmarks/710_hebbian_karc_readout_goat.md`](../../.benchmarks/710_hebbian_karc_readout_goat.md). |
...
| `certified_frontier` | `katgpt-core/certified_frontier` | Modelless safe-set expansion for binary verifiers over latent cells (Plan 580, Research 510, arXiv:2606.08802): monotone certified cell set + Lipschitz reachability dilation + safe uncertainty acquisition + halting law — "where do I look next" + "when do I stop". Both factorisations of the linear-kernel posterior behind one `LinearPosterior` trait (primal `PosteriorBuffer` for n < D; dual incremental-Cholesky `DualPosteriorBuffer` for n > D — 69–84× faster at n=256/D=32, O(1) in n). **Opt-in** — GOAT G1–G4 PASS ([Bench 688](../../.benchmarks/688_certified_frontier_goat.md)): 0.264 µs/query vs 1 µs (13.2× iterated), 0 unsound certifications over 1000 adversarial seeds, halting law at the predicted 1/ε². T3.4 SPLIT — the plan's product metric measured degenerate; promotion = re-gate on a corrected metric, consumer-pending. Fuses with `viable_manifold_graph` as its acquisition half. |
...
| `set_admission` | `katgpt-core/set_admission` (implies `certified_frontier` + `spectral_pencil` + `distributional_steering`) | Counter-anchored set admission — the R4T counter-anchor reward extracted as a **selection-time set operator** (Plan 599, Research 564, arXiv:2603.06397; the trained twin is riir-train Plan 412, SECONDARY by the serving-envelope rule). Greedy admission over `g(x) + α·cos(x, q₀) + κ·log(1 + x̂ᵀM⁻¹x̂)` — quality + counter-anchor alignment + exact marginal log-det gain via the `M ≡ I + G` ridge-dual, `M⁻¹` maintained by Sherman–Morrison rank-1 (O(d²)/admission at d=8) — under the `ColinearityBatchGate` 0.95 cap. Certificate EXACT, not sampled: the d×d dual Gram diagonalizes through `spectral_pencil::dense::jacobi_eigen` → `certified_frontier::vendi_diversity` (both substrates consumed, neither forked; the K×K Gram never built), + incremental participation-ratio collapse tripwire between certifications. Classical-DPP scoping honest in-source: the (1−1/e) bound applies to the surrogate; exact Vendi is the honesty instrument. **Opt-in — GOAT G1–G4 PASS** ([Bench 811](../../.benchmarks/811_set_admission_goat.md)): step 33–39 ns (5× headroom on the 200 ns budget; gate + certificate budgets re-derived with evidence, Plan-306 precedent), G1 greedy==OPT 8/8 brute-force worlds, L1/L2 property tests pin the three degenerate interiors (paraphrase-collapse / semantic-drift / coordinate-gaming), G4 zero-alloc over 1000 cycles. **NOT promoted — Phase 4 consumer eval is the promotion gate** (riir-clippy issue-121 span-pool rerank; the ndb counter-anchored retrieval lane). Full narrative: [catalog §115](../09_feature_catalog/opt_in_features.md) |
...
| `incidence_algebra` | `katgpt-core/incidence_algebra` | The thought × agent incidence-mask algebra (riir-ai Issue 874 T1 / riir-ai Research 364, arXiv:2510.20733 ThoughtComm distilled): the who-shares-what mask becomes a first-class object — `agreement_counts_into` (αⱼ per-thought witness counts) + `agreement_score` (one monotone σ-saturated tier curve, α≤1 anchored to EXACTLY 0.5) with two consumer maps that never gate: `routing_weight` (α=1 is bit-identically the unweighted path — the Bench-013 soft-bias lesson) and `contagion_strength` (α=1 is EXACTLY zero crowd strength — the Plan-019 CLR stampede fix); shared/private split + `private_fractions_into` (never report agreement without the retention counter — collapse = conformity, paper Appx C.2); Hall feasibility (`hall_max_matching_into`, Hopcroft–Karp, zero-alloc scratch); `audit_mask` with the DENSITY_ALERT warning (a dense mask is the crowd-panic precondition); `rank_by_agreement_into` (α desc, index asc — the Issue-849 tie-break lesson). Agent-major row-major layout, zero hot-path alloc. Think-brain local — masks/α/tier weights never sync; contagion intensity and witness counts cross raw. First consumer: riir-games-shared `agreement_tier` → riir-stealth `agreement_contagion` (Issue 874 T2). **Opt-in, no standalone GOAT** — the substrate's 11 module unit tests gate the algebra; the consumer A/B verdict lives on the stealth side. |
