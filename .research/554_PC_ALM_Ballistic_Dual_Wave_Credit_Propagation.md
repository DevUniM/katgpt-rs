...
- **riir-clippy `select_best_candidate` / `EvolveRecorder` / fixseq ring**: selection consumes **terminal outcomes** (`W_EVO·evolution + W_RATE·reliability`); the ring already records per-attempt `errors [b,a]` — the raw material for an **integral-of-journey** credit `λ_T = α·Σ_t r_t` exists, but nothing accumulates it. This is the documented strand-keep poison (Bench 089/090): a KEEP ending at a masked E0614 scores as success — terminal residual ≈ 0 while the integral is large. → [riir-clippy Issue 105](../../riir-clippy/HISTORY.md).
