...
**PoC Addendum (2026-07-07, riir-ai Issue 373):** The `belief_share` PoC was filed, scoped, and run. **Verdict: DEFENDED — personal signal survives.** The T1 architectural audit found that post-action routing and the personality composition kernel are structurally per-entity (no cross-entity aggregation); the only dilution site is the COMMUNITY layer (`riir-ai/crates/riir-engine/src/entity_cognition/layers/community.rs`), which reads a shared faction-culture vector from `FACTION_CULTURE_CACHE`. The PoC (`riir-ai/crates/riir-poc/benches/belief_share_npc_dilution_poc.rs`) measured `belief_share = ‖personal_output‖ / ‖full_output‖` across 100 NPCs with N=9 layers, sweeping `w_community ∈ {0, 0.5, 1.0, 1.5, 2.0}`, faction-culture strength, and `faction_confidence` decay. **Result: belief_share ≥ 0.98 across ALL configs**, including the faction-dominated worst case (`w_community = w_max`, strong faction, fresh observation). The faction culture contributes at most ~1.7% of the composed output norm because 8 personal layers collectively outweigh 1 faction layer. **`belief_share` is a pure diagnostic — no behavior change, no GOAT gate, no modelless correction needed.** Two sub-findings: (1) `sigmoid(0) = 0.5` means neutral weight is half-contribution, not zero (to fully suppress a layer, weight must be strongly negative); (2) a diffuse faction culture has lower belief_share than a sharp one due to geometric alignment with personal directions. See `riir-ai/.issues/373` §7–8 for the full verdict table and findings. The claim that "the paper's dilution transfers to per-NPC cognition" is **refuted at the architectural level** — our N=9-layer per-entity structure does not have the softmax-over-many-keys dilution mechanism the paper identifies.
...
> **Gain cross-ref:** Litman [arXiv:2601.15380 "You Need Better Attention Priors"] — signal-diff run per the §3.6 discipline (Research 566 §2): SSMax's Δ is a **content** margin (gold-vs-distractor gap, multiplicative temperature, gold-mass preservation) while GOAT's δ is a **prior** margin (additive structural lane, context-noise-bleed bound) — different signals, different axes, both exponential-in-margin laws, and they compose (SSMax on the content channel, the prior-logit lane on the structural channel). Sigmoid-gate variant of the margin law filed as Research 566 + Issue 819.
