...
> **PASS-Redirects (synthesis):** Bargatin et al. [arXiv:2609.11486 "FreeFlow: A Bias-free Hierarchical Transformer for Optical Flow Estimation"] — the opposite pole on this note's axis: strips ALL task-specific inductive biases (correlation volumes, warping, iterative refinement) from optical flow via a plain window/shifted-window/global-at-low-res hierarchical transformer and still reaches SOTA (ECCV 2026). PASS: vision task with zero consumer surface in the stack; the attention topologies are Swin/SegFormer 2021 prior art already covered LLM-side by DashAttention/HLA/GDN2; and the paper's win condition (abundant capacity) inverts under our µs-budget economics, which deliberately favor biases (rules, pruners, entmax routing).
