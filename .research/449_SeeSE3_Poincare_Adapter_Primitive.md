...
> **PASS-Redirects (synthesis):** Bargatin et al. [arXiv:2609.11486 "FreeFlow: A Bias-free Hierarchical Transformer for Optical Flow Estimation"] — trained CV model that ESTIMATES optical flow (dense pixel correspondence via a bias-free hierarchical transformer, ECCV 2026 SOTA); no bearing on this note's geometric flow theorems (rotational optical flow depth-independence — that is scene geometry, not an estimator). PASS: no optical-flow/pixel-stream consumer surface anywhere in the stack (NPC perception is symbolic: MapPos + visible_radius + zone KG), and vision-transformer training is out of riir-train scope (text LLM + code LoRA + game trajectories only).
