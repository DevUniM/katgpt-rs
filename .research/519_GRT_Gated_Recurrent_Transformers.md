...
> **PASS-Redirects (synthesis):** Yifan Zhang [techreport "Recurrent Looped Transformer" (github.com/yifanzhang-pro/recurrent-looped-tranformer, Sept 2026)] — the loop family's cross-token (temporal) member: final-state sigmoid-gated feedback carried across prompt+response over encoder-KV + SWA, tied 48+48. Architecture PASS (family ships — this note's GRT is the depth-loop member; RLT's recurrence is across tokens, the Feedback-Transformer/Block-Recurrent shape, ingredient-wise anticipated; zero measured results in the report). RL-replay correctness contract distilled to riir-train `.research/453` + Issues 543/938.
