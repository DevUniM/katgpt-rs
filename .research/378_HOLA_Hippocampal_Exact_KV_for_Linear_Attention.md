...
**PASS-Redirects (synthesis):** Jolicoeur-Martineau, Sukthanker, Cameron & Gervais [arXiv:2608.28444 "Sliding-window beats linear attention"] (Microsoft ASG, Aug 2026) — cross-evidence for HOLA's premise from the attention side: training-free SWA(w, s=4 sinks) recovers 99.0% of teacher short-context average and beats post-trained linear attention (LoLCATs/Liger-GLA) 2–10× on S-NIAH/BABILong — a linear recurrent state alone cannot do long-context recall, with or without post-training. Benchmark-design consequence for the deferred G5 gate: when it runs, the baseline battery must include a SWA+sink arm (the cheap training-free comparator this paper establishes), not only full-attention and GDN-alone. No policy change: this stack never retrofits linear attention (all linear surfaces — GDN2/KDA, DeltaNet hybrid, HLA — are native/from-scratch), and SWA+sink already ships as zero-training layout (rt_turbo SWA(8192,4), FlashPrefill sink blocks, Gemma-4/DFlash2 native rings).
