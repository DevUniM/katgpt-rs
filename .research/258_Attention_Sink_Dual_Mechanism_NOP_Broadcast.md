...
> **PASS-Redirects (synthesis):** Muyu He ["Disabling Attention Layers", riddlehe.github.io/blog/disabling-attention-layers.html, 2026-08/09 blog] — attention **contagion**: a query's attention mass to a target token is causally supported by OTHER queries' attention to that same target (proved by attention-score patching: restoring only the last token's scores to the entity fully recovers entity extraction after intermediate tokens' access is cut). A cross-position causal-coupling complement to this note's static sink-concentration story — sink/heavily-attended rows are load-bearing partly because they are attended. Supports never dropping sink rows from KV (`shard_kv` sink_tokens, q8kv sink guard); no runtime surface here edits per-position attention scores (SP-KV gate bias is validated end-to-end), so nothing to action.
> **Gain cross-ref:** Litman [arXiv:2601.15380 "You Need Better Attention Priors"] — EOT derivation (sinks = optimal KL-regularized transport defaults) + a **quantitative margin law** this note's classifier lacked: prior-logit margin δ bounds context-noise sensitivity ≤ (L−1)/(e^δ+L−1), δ ~ ln L suffices → extends the NOP/Broadcast classifier from descriptive to **predictive** (measured margin forecasts the context length a sink head protects). Filed as Research 566 + Issue 819 T1 (`SinkDiagnostic.margin` + `forecast_stable_positions`).
