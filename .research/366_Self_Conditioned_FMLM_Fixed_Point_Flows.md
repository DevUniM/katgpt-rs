...
> **Cross-ref (2026-09-16):** Tang & Wang [arXiv:2609.15903 "Discrete Beckmann Transport Models"] — the DISTILLATION-FREE discrete sibling of FMLM★: same autonomous-velocity/one-step-map thesis, but trained by conservation-equation residual (no CDEQ teacher, no two-time semigroup distillation) with confidence-commit refinement as the NFE axis. The shipped inference surface this note documents (LoopMode/is_converged/RCD/3SR) covers its iteration half; the deltas route to [Research 563](563_DBTM_Discrete_Beckmann_One_Step_Language.md) → Issue 811 (modelless) + riir-train Plan 411 (recipe arm, sibling of Plan 395).
