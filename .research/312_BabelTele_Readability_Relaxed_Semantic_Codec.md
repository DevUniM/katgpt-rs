...
> **riir-agents consumer resolution (2026-09-03):** the `SharedContextEntry` POD already IS the readability-relaxed codec for the modelless fleet (latent_dir + verbatim ref-tags + BLAKE3 — strictly stronger than BabelTele's empirical decodability on the verification axis). The residual niche — dense-text payloads for an LLM-`Reasoner` fleet (encode ≈ free, rides the authoring generation; heterogeneous/black-box readers) — filed as riir-ai `.issues/852_babeltele_llm_reasoner_fleet_dense_gist_payload.md` (conditional Gain, gated on a materialized consumer). The deterministic-codec value path above was G2-failed at 1.14× (Bench 331).
