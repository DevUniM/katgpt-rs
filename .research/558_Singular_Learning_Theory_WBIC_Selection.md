...
**Status:** RECORD + LANDED — the Track-A half shipped as `katgpt_core::slt`, promoted default-on (`580bda30`, Bench 764 GOAT, 2026-09-15). The T0 novelty verdict on the §2.3 noise-sweep estimator: KEEP at application level (form = classical Hill estimator — caveat recorded in HISTORY §Issue 781). T4 LANDED 2026-09-15 as `slt::sweep` (`slt_sweep`, default-on; Bench 765 GOAT) — the shipped form is the shell-cancelling log-log CDF slope, NOT the Hill window (the Gaussian probe measure breaks the exact-power-law premise; −28…−37% systematic, the v1 record); tube/cone geometries near-unbiased (RRR −1.1%), star bodies −18% at d≥4, feasible domain λ ≲ 3, and the LOCAL tangent-cone exponent ≠ the tempered-global RLCT (ReLU toy: local ≈2.77 vs SGLD 0.526 — §2.3's forward-pass estimator measures the local cone slope, not the posterior-weighted value). Track-C half = riir-train Plan 404 (unclaimed).
