...
| 3 | Corpus-grounded generation (anti-collapse) | Sample doc → deterministic seed → generate; Vendi/effective-rank as diversity metric | **Partial** — `RpgQuestCorpus` is a frozen corpus (drafter samples it); no fresh-doc-grounded GENERATION; `ShardIndex::retrieve_diverse` enforces diversity at retrieval | Hash-seeded generation + effective-rank tripwire |
...
| `RpgQuestCorpus` + `AdaptiveQuestDrafter` | frozen corpus → hash-seeded draft (42 ns) | fresh external doc per round → generate NEW content | Ours samples existing; SPADE's grounds creation. **Partial** |
