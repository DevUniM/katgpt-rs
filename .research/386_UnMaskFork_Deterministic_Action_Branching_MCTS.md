...
> **PASS-Redirects (synthesis):** Park et al. [arXiv:2609.21032 "Scaling Discovery through Test-Time Communication"] — their Prop 1 (`T_team = Σ_j min_i X_ij ≤ min_i Σ_j X_ij = T_best`) is Eq 1 with the axes swapped: (agent × stage) instead of (action × state), plus Erlang/Chernoff tails — Eq 1 has priority (Feb 2026 vs Sep 2026). Their additive content is the empirical multiplier (team@k ≈ best@4k–6.6k) + the herding correction g ≤ k. See [.research/583](583_Test_Time_Communication_Verified_Progress_Sharing.md).
