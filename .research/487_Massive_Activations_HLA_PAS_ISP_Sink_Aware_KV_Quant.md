...
**PASS-Redirects (synthesis):** SMELT [arXiv:2609.01343 "SMELT: Scaling Laws for Compute-Matched MoE Looped Transformers"] (Wang & Zhang et al., Tsinghua/ByteDance Seed/M-A-P, Sep 2026) — cross-visit evidence FOR the write–sink–cancel reading: in trained looped-MoE models the second weight-tied visit broadly reduces attention-sink mass (BOS 0.60→0.02 on the Dyck head, REVERSING the Baseline's sink-grows-with-depth trend) and reallocates it to content tokens while Q/K retrieval coordinates stay fixed (cos 0.89–0.93) and V diverges (0.65–0.74) — i.e. sink mass is transient reallocatable parking, not semantic content, exactly the lifecycle this note distilled. No policy change: SMELT studies TRAINED looped models under from-scratch pretraining (→ riir-train; PASS-redirected at Research 073); the frozen non-looped checkpoints this stack serves (Bonsai/qwen38/gemma) keep their persistent-MA sinks, so the sink-sidecar Q8KV policy (Issue 716) stands unchanged.

**PASS-Redirects (synthesis):** Jolicoeur-Martineau, Sukthanker, Cameron & Gervais [arXiv:2608.28444 "Sliding-window beats linear attention"] (Microsoft ASG, Aug 2026) — independent cross-evidence for sink criticality at the mask level: applying SWA to full-attention-trained models is catastrophic once sink tokens leave the window, and retaining just s=4 sinks restores 99.0% of teacher average at zero post-training — the same "sinks are load-bearing infrastructure" conclusion this note reaches at the quantization level (PAS/ISP carriers). The sink-position quantization prior (Issue 716 f32 sidecar) stands unchanged; no policy change.
