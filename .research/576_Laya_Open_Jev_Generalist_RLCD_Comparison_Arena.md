...
| **Confidence via similarity-to-training-data** ("knows when it doesn't know") | a | **NOT shipped — the note's one real extraction**: corpus-distance confidence is computable modellessly — retrieval distance from `riir-rag` (BM25 rank / KNN cosine) → sigmoid gate → abstain when the state is far from every registered corpus region. Our ABSTAIN is score-threshold-based only; a distance-based arm is a genuine small primitive (kNN-density abstain) | **Gain — candidate for the engine's abstain signal** (Proposal 014 T1.6 — its own task, marked UNVALIDATED) |
