...
> **Cross-ref (riir-train, revised 2026-08-07):** Wang et al. "Zero Train-Inference Mismatch — now for linear attention, and under async RL" [blog https://yichuan-w.github.io/blog/GDN-train-inference-mismatch-asyncRL/ — Gated DeltaNet bitwise trainer/generator parity on TorchTitan RL + vLLM]. **Verdict revised PASS → Gain** after user pushback ("we have both modelless and model-based"): the model-based track is broader than Plan 318 SFT — it includes Plan 059 G-Zero DPO/GRPO (RL training) + Plan 066/distill_attention.rs (SDPA→HLA/AHLA distillation). Per §3.5 Path 0.5, applicable training-efficiency papers get a Plan in riir-train. Technique is conditionally applicable: current HLA distillation is batch-invariant by construction (CPU recurrent forward, single sequence, FD gradients — no chunked backward to drift from), but WILL bite on GPU-batched HLA training + async RL. → riir-train/.issues/420 (verify batch-invariance), riir-train/.plans/327 (async-RL parity recipe).
