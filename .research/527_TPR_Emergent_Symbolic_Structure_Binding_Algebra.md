...
**Fusion F6 (consumer, riir-ai — the #1-priority surface, filed as riir-ai `.issues/847`; SHIPPED + PROMOTED default-on 2026-09-03 in riir-ai `317a54ae8` — Bench 852 GOAT 10/10, OOD-A wall closed 8/8 vs 0/8, withheld-BINDING wall honestly 0/2; the issue is closed + removed, record = riir-ai `.plans/565`; this made riir-games-quest's default path the first DEFAULT-PATH consumer of katgpt-core `tpr`, which changes the premise of the Bench 698 T-Promote row): quest_grammar role-move variants over S-V-O.** The paper's Figure 7.4C role moves (adjective / PP / relative clause relocated between subject- and object-modifier roles, with the agreement cascade — the verb re-inflects because the moved clause changed heads) are quest-text transformations, and the substrate already carries every load-bearing piece: `GrammarValidatedOutput.svo_triples` is the paper's OWN role scheme (§5.4: S-V-O roles gave DISCOVER perfect approximation), `RpgGrammarAnnotation.verb_forms` is the agreement surface the cascade rewrites, `GrammarValidator` is the downstream re-validation layer structural moves require ("identity local, structure distributed" — batch-edit every slot encoding carrying the moved element), and `grammar_training.rs` is the corpus/LoRA surface for the withheld-(noun, slot)-pair OOD bench (§9.5's COGS novel-role failure = the drafter's coverage wall) and the L2,1 recipe (`GrammarLoraHeader`). Two consumption modes: construction (compose quest latents as Σ bind(fᵢ, rᵢ) over S-V-O; surgery = variant generation without corpus coverage of the variant, validator never bypassed — surgery proposes, the validator disposes) and certification (ALS fit over `TernaryDraftModel` latents to certify the binding the drafter actually learned; unbinding = slot readout).
