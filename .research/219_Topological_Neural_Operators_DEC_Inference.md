...
> **Cross-ref (Gain):** Lee/Coeurdoux/Potaptchik/Du/Albergo/Vanden-Eijnden [arXiv:2608.01692 "Beckmann Transport Models: From Autonomous Flows to One-Step Maps"] — generative-modeling theory paper whose core equation `∇·(νb)=μ0−μ1` is the divergence `δ` (= `codifferential`, this note's DEC substrate). **Revised from PASS to Gain** (Research 468): BTM interprets it as a flux constraint giving a dynamic realization of Beckmann's transportation problem — and this IS the "occupation-measure flow constraint" our shipped CCE Moderator (Plan 295) documents as missing. The DEC `δ` operator is the modelless analog. The paper's training (regression loss for `b`, map loss for `T`) → riir-train; the generative application (image FID) is out of scope. See Research 468 for the CCE-MFG fusion + Research 296 for the explicit Stokes/DEC vocabulary crosswalk.
> **PASS-Redirects (synthesis):** Ge, Liu, Wang, Garnica, Lyu, Wang, Tan, Gao, Zhang, Hong, Wu, Li [arXiv:2609.11900 "MindTopo: Can Foundation Models Reason in Topological Space?"] — benchmark of 14 MLLMs on five Piagetian topological invariants (continuity, separation, order, enclosure, knots): best model 61.4% vs human 97.9%, and hard-tier hole-counting (exactly `b₁` = this note's DEC `betti_numbers`) collapses to ≤2.4% on every model while maze reachability (π₀ components over the free region) is where open-weight models sit at chance. External measured evidence for the compute-don't-learn posture: the invariants this substrate computes exactly from explicit cell complexes are the ones learned models extract unreliably from pixels. PASS — we build no vision models and no actionable gap exists (our game worlds ARE complexes/graphs, not renderings); the paper's SFT+GRPO recipes are VLM-specific (out of scope, no VLM pipeline in the stack) and its own measured result — training lifts planning only 0.2%→6.3% — supports the modelless-planning posture rather than contradicting it.
