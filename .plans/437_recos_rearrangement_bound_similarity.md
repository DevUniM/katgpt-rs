...
**Target:** `katgpt-rs/crates/katgpt-core/src/similarity.rs` (open primitive) + `katgpt-rs/crates/katgpt-core/src/mag/` (cold-path consumer) + `mmorpg-remaster/crates/mmorpg-asset-explorer/src/index.rs` (conditional hot-path consumer)
...
|---|------|-----|-----------|------------------|--------|
| 1 | `ShardIndex::query` ±1 rerank (hot) | 8 | `mmorpg-remaster/crates/mmorpg-asset-explorer/src/index.rs:253,258,265` | `cosine_sim_ranking_scaled(ctx, &emb, norm_a_sq)` | ✅ verified |
| 2 | `query_k_nearest_cosine` scoring (hot) | 8 | `mmorpg-remaster/crates/mmorpg-asset-explorer/src/index.rs:1041` | `dot_8(&ctx_norm, &normalized_hla[..])` (unit-norm → cos=dot) | ✅ verified; note didn't emphasize |
...
- [x] **T1.2** Add `recos_sim` to `similarity.rs`, gated `#[cfg(feature = "recos")]`:
  ...
  ```
  `dot_8` is private in `mmorpg-remaster/crates/mmorpg-asset-explorer/src/index.rs`; katgpt-core's `similarity.rs`
...
- [x] **T4.2** In `mmorpg-remaster/crates/mmorpg-asset-explorer/src/index.rs` `ShardIndex::query`, added a
