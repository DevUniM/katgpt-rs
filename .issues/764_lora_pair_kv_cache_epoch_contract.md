...
**Status:** OPEN — contract gap, no wrong output ships today (the example demonstrates the mixed-epoch read as a feature without claiming exactness); the gap becomes a bug the moment any consumer builds a scored replay / deterministic re-run / verified regeneration on a hot-swapped cache.
