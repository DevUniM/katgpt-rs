...
**Status:** RESOLVED 2026-09-19 — T1 filed with the measurement, **T2 repaired and verified** (5 of 5 arms print a non-zero quantity, every bar unchanged), **T3 COMPLETE — all 34 asserting timed regions at n ≥ 1000 have now been RUN** (10 + 24; **7 VANISHED, 27 SURVIVED, 0 UNMEASURED**; all 7 repaired and re-run, every bar unchanged, every full-target pass count unchanged); **T4 CLOSED** — the proposed `let _ =` predicate is REFUTED by the execution run (20.0% vs a 21.1% base rate) and `scripts/timed_region_guard_gate.py` shipped in its place as a docs-gate CHECK (membership wall over the READ tier, ratchet over the unread one); **T5 RUN — all 33 sibling rows executed 2026-09-19** (1 UNBUILDABLE, 32 executed, **2 VANISHED = 6.3%** against this repo's 20.6%; both repaired in riir-ai `3712d51b6`, issue `986` there; riir-chain `039728c8` filed for the unbuildable one). Three new buckets fell out of the run — PRINTS-NOTHING, `#[ignore]`d, UNBUILDABLE — none foldable. The **ratchet-only sweep half landed the same day** (T6) — `timed_region_drift_sweep.py`, 21 repos. **Every task closed.**
...
- [x] **T6 — the cross-repo sweep half, RATCHET-ONLY. DONE 2026-09-19** — `scripts/timed_region_drift_sweep.py` + `scripts/timed_region_drift_floors.txt`, 21 repos, ratchet at measured, two floors that break separately + a reserved `TOTALS` row (8 of 21 repos have 0 regions, so both per-repo floors are vacuous there). The ratchet and the walk floor were each PROVED to fire. ⛔ The two reported buckets required extending the shipped classifier, and the extension found its own defect: `mask_file` BLANKS `#[...]` lines, so an attribute run read off the masked text is always empty and the `#[ignore]` bucket would have reported a confident ZERO. Attributes are read off the RAW text at the masked offset — safe only because the masker is length-preserving, which is now an ARM rather than an assumption. ⚠ `sweep_advisory`'s lines already carry their own glyph, and this sweep's first run printed `⚠ ⚠ STALE:` — the doubled-glyph defect `deferral_line` exists to prevent, reproduced within minutes of reading its docstring. Original framing follows.
