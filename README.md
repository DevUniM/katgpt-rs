...
A **GOAT-proved** neuro-symbolic micro-Transformer with speculative decoding, constraint pruning, and **636 feature flags (204 default-on, all GOAT-proved)** — built in Rust. Pure algorithms, zero side effects, MIT licensed.
...
The default production stack has **204 GOAT-proved default-on features** (636 total flags), but they don't all run on every token. The architecture uses **layered gating** — most features are bandit-driven, Option-gated, or compile-time-only.
...
### 🧠 KARC Hebbian Readout: the `w_out` Fit, Hebbian-Flavored (Plan 584)

`KarcForecaster::fit_ridge` and `HebbianKernelMemory::construct` are the **same ridge closed form with different feature maps**. This primitive applies the fact-store thesis to the KARC readout: training pairs become facts (key = `pad64(delay_state)`, value = `pad64(target)` — zero-padding is inner-product-exact), and the constructed (A, G, B) plays the `w_out` role. What the unification buys over `fit_ridge`: **edit semantics** (the fact-edit journal loop applies to forecasting), the **per-fact margin audit** (γ_min per key), the riir-neuron-db freeze chain (`HebbianConstructedShard` → the KARC-identity two-block envelope), and the canonical value table (observed trajectories ARE the values). f32 throughout (`fit_ridge` solves f64 — cross-arm comparisons state an f32 tolerance).

**GOAT status (Bench 710, ALL PASS):** G1 five gates (pair retrieval at belief scale, bit-identical determinism, pad64 exactness, F=1 infinite margin, error shape) ✅. G2a per-fact fit ratio **0.7× — the Hebbian fit is FASTER than `fit_ridge` at runtime scale** (10.6 vs 15.7–15.9 µs/fact; the m=128 f32 closed form beats the d_h=256 f64 Cholesky while carrying the margin audit) ✅. G2b forecast wrapper Δ 0.5 ns ✅. G4 0 allocs ✅. Honest notes: `fit_ridge` turns rank-deficient below d_h (discovered by the run); the arms' data shapes differ by construction.

**Decision: stays opt-in (consumer-first promotion rule).** The math is generic and IP-free like `hebbian_kernel_memory`, but both private consumers — riir-neuron-db `karc_hebbian_envelope` (Plan 584 Phase 2) and riir-ai `karc_hebbian_bridge` (Phase 3, Bench 909) — stay opt-in pending the swarm-homeostat production wire (owner call); default-on now would repeat the `similarity_inference` 24-days-zero-consumers demote (Issue 867).

Feature gate: `karc_hebbian_readout` (**opt-in**; implies `hebbian_kernel_memory`; zero runtime cost unless constructed). 📖 Plan: [riir-ai `.plans/584`](../riir-ai/.plans/584_karc_hebbian_wout_unification.md). Bench: [`.benchmarks/710_hebbian_karc_readout_goat.md`](.benchmarks/710_hebbian_karc_readout_goat.md). Module: `crates/katgpt-core/src/karc/hebbian_readout.rs`.
...
| **NonergodicFilter** (`nonergodic_belief`) | Two-level nonergodic Bayes filter (Plan 592, Research 545, Simplex blog 2026-09-09): K per-generator inner beliefs η_n + online posterior w over generators coupled by one LSE normalizer; telescoping `w_n·η_n` readout; commit/revive semantics. K≤16, D≤64, zero-alloc. | Opt-in — Bench 706 G1–G4 PASS (564 ns/tick @ K=8/D=8, 0 allocs; exact vs brute-force 1e-5); beats tuned-leaky + BoM hard-select on log-loss. Stays opt-in until a riir-ai runtime-consumer GOAT (Guide 373). |
| **SaddleEscapeGate** (`saddle_escape`) | Three-way Continue/Halt{Trapped}/Kick loop control for looped latent reasoners — wraps the Plan-304 `GainCostLoopHalter`; `FlipDetector` decode-flip EMA + oscillation streak + BLAKE3-seeded escape kick (Plan 593, arXiv:2609.04963). First consumer: riir-ai Plan 582 deliberation flip leg. | Opt-in — GOAT G1–G5 PASS (Bench 707: 5.3 ns/loop Continue, 0 allocs); promotion owner-gated on a real-model-trace demonstration |
| **GWAlignment** (`gw_alignment`) | Gromov–Wasserstein quotient alignment of two precomputed distance matrices (n,m ≤ 64) — structure-only cross-space correspondence, no shared coordinates, no correspondence prior; multi-start init + product-graph power iteration, sum-exact loss, bit-deterministic (Issue 743 / Plan 594, riir-ai Research 371). | Opt-in — GOAT G1–G4 PASS (Bench 709: per-level AUC ≥ 0.96 through ε=0.10, 0 steady-state allocs); stays opt-in per the consumer-first rule (zone-belief consumer PoC = riir-poc follow-up) |
| **MarginalRewind** (`marginal_rewind`) | Calibrated noise-backtrack for interpolant flow states — rewind the CURRENT state to an earlier confidence level at the exact marginal variance (`a = s/t`, `sqrt((1−s)²−(a−s)²)`; identity `a²(1−t)²+(1−s)²−(a−s)² = (1−s)²`), no clean-point knowledge; γ-dial for continuous rewind depth. The reactive recovery operator cgsp collapse detection lacks (Issue 746 Row 2, arXiv:2609.11801 Eq 18). | Opt-in — Bench 712 defend-wrong PoC PASS: **5.71×** vs additive-at-equal-budget, dominates hard restart at lower budget in the collapse-prone regime (26.1% @ σ=0.74 / 41.7% @ σ=0.90 vs 20.9% @ σ=0.95); restart wins clean priors (the consumer decision rule); 0 allocs, RNG-free. Promotion owner-gated on a live consumer |
| **MetabolicGate** (`metabolic_gate`) | Energy-coupled compute gating — the compute-budget family's **stock axis** beside `gain_cost_halt`'s utility flow: `depth_factor σ((stock−base)/scale)` compute-depth multiplier over a metabolic stock; the Theorem-1 starvation design law `2ε < L(1+(1−α)δ)` (`defector_starves`); the Lemma-1 break-even inefficiency `K(ε,L)` (32-iter bisection, `metabolic_drag_threshold`); conservation-exact `steal_lossy` (system destroys `(1−α)δ`); lottery-share `execution_share` (both-zero → ½). Zero-alloc f32, NaN-never-fires. (Research 550, arXiv:2609.10817 "Tapes Together Strong"; first consumer: riir-ai Plan 585 — thermal-LOD metabolic axis + swarm energy commons.) | Opt-in — GOAT G1/G2/G3/G4 ALL PASS (Bench 716: K-solver residual 6.7e-6; depth 3 ns / share sub-ns / K-solver 412 ns bounded; 0 allocs; regime tuple ε=12 L=10 α=0.3 δ=3 viable+starving). Stays opt-in per the no-default-consumer rule — Plan 585 re-gates at Phase 4 |
| **RefinementMarginal** (`refinement_marginal`) | Single-pass token→byte distribution conversion with the terminal-mass certificate (Plan 598, Research 559, arXiv:2609.12303 Marginalize-It family): 257-bin record (256 byte bins + a TERMINAL bin that ABSORBS the boundary mass the classic pass renormalizes away), closed-form `error_bound` (TV ≤ M/(1−M), over-reports by construction), `expected_escalation_cost` (Σ_k M_k), `escalation_sigmoid` serve gate; streaming frontier (one pass over the flattened table worst case, zero allocs). BPE bridge in the root crate (`refinement_bridge`: table build + `tokenizer_geometry` + `decode_argmax`). | Opt-in — Bench 770: G1/G3/G4 PASS (5 unit tests + 98-cell integration vs independent reference, bound held 100%, 0 allocs) but **G2 FAIL honest** — the byte gather is 1.97× softmax at depth-0 alone (2.63× full loop), refuting the <5% premise; NOT promoted. The proven certificate is inherited by any future exact-conversion lane |
...
| **Leakage Probe** (`leakage_probe`) | Modelless attribute-transfer leak score for stored vector stores — defender-side audit across TWO embedding spaces with no encoder access (Issue 736, Research 540; threat model arXiv:2505.12540 vec2vec). GD-free transport (subspace iteration → PCA whitening → CSLS-corrected Sinkhorn → orthogonal-Procrustes polar factor via `linalg::symmetric_eig`, deterministic multi-start) + kNN attribute-transfer top-1 vs majority-class chance + neighborhood-hit + alignment diagnostics. | Opt-in — in-module G1 planted-leak recovery + G1b monotone-in-noise + G1c iid-labels control + G2 smoke ALL PASS (10/10 tests). Consumer: riir-neuron-db Issue 614 E2 landed (Bench 495: planted recovery top1 0.828 vs chance 0.086, lift 9.64); no katgpt-rs GOAT bench claim (the `cond_audit` precedent). |
| **Convergence Cadence + Loop Residual Exit** (`cadence_gate`) | Windowed ‖Δz‖ Settled/Churning classifier (`ConvergenceCadence`, Issue 720 / Research 529) + `LoopResidualExit` — the residual-gated early exit for the weight-tied `forward_looped` (Issue 731, EqR action item 7.2): magnitude (L=3 window mean < τ) OR shape (`Settled`) arms, `d_min` floor, caller-owned probe slot (`None` = bit-identical to fixed-D), `with_cadence_config` consumer floor-calibration seam (T5, motivated by riir-ai 881's CCE-scale residuals), `with_shape_persistence` (T6: ≥2 consecutive Settled windows — closed the rule-3 decay fall-through false-positive class, 40 → 0 control fires, 12× better worst-case exit dist). | Opt-in — T2 knee k=10, settle-leads-knee; T3/T6 synthetic G2 = **existence-proof grade** (v4 seed + held-out 1002; P2 1/12); promotion deferred on real-workload evidence (riir-ai Proposal 045). T4 zero-alloc `[f32;4]` ring; G1 fed-but-never-firing ≡ None bit-identical. |
...
| **Usage-Rate KV Eviction** (`usage_rate_eviction`) | Mass/age eviction scoring `cum_mass / max(1, age)` (Plan 585, arXiv:2608.19920 H2O-normalized) + the `runaway_gate` generation-runaway canary + the `beats_random_prompt_pin` null-control bar (Bench 697 addendum, Research 531). Standing promotion rule for any lossy KV policy: `runaway_gate` ∧ `beats_random_prompt_pin` ∧ the protection factorial. | Opt-in — MIXED GOAT (Bench 697): G1–G4 PASS, 2–4× raw-H2O recall at cap ≥ 32, honest miss at the 8%-budget extreme; no consumer = no promotion (Issue 836 pull-gated). See [catalog §91](.docs/09_feature_catalog/opt_in_features.md). |
| **Certified Frontier** (`certified_frontier`) | Modelless safe-set expansion for binary verifiers over latent cells: monotone certified cell set + Lipschitz reachability dilation + safe uncertainty acquisition + a halting law (Plan 580, Research 510, arXiv:2606.08802) — "where do I look next" + "when do I stop". Ships BOTH factorisations of the linear-kernel posterior behind one `LinearPosterior` trait: primal `PosteriorBuffer` (n < D) and dual incremental-Cholesky `DualPosteriorBuffer` (n > D, 69–84× faster at n=256/D=32, O(1) in n). | Opt-in — GOAT G1–G4 PASS (Bench 688): 0.264 µs/query vs the 1 µs budget (13.2× iterated from a 3.428 µs first FAIL — SoA lane + branch-free argmax), 0 unsound certifications over 1000 adversarial seeds, halting law fires at the predicted 1/ε² (185/728/2961 obs for ε = 0.2/0.1/0.05). T3.4 floor gate SPLIT — the plan's stated product metric measured DEGENERATE; promotion deferred to a re-gate on a corrected metric (consumer-pending). Fuses with `viable_manifold_graph` as its missing acquisition half. See [catalog §116](.docs/09_feature_catalog/opt_in_features.md). |
| **Counter-Anchored Set Admission** (`set_admission`) | Selection-time set operator from R4T (Plan 599, Research 564, arXiv:2603.06397) — no RL, no trained weights: greedy admission scoring `g(x) + α·cos(x, q₀) + κ·log(1 + x̂ᵀM⁻¹x̂)` (quality + counter-anchor alignment + exact marginal log-det gain via the `M ≡ I + G` ridge-dual, Sherman–Morrison rank-1 at O(d²)/admission) under the `ColinearityBatchGate` cap. Certificate is EXACT (d×d dual Gram → `spectral_pencil::dense::jacobi_eigen` → `certified_frontier::vendi_diversity` — both substrates consumed, neither forked) + incremental participation-ratio tripwire between certifications. The trained twin is riir-train Plan 412, SECONDARY by the serving-envelope rule. | Opt-in — GOAT G1–G4 PASS (Bench 811): step 33–39 ns (5× under the 200 ns budget), greedy==OPT 8/8 brute-force worlds, L1/L2 property tests pin the three degenerate interiors (paraphrase-collapse/semantic-drift/coordinate-gaming), G4 zero-alloc over 1000 cycles. **NOT promoted — Phase 4 consumer eval is the promotion gate** (riir-clippy span-pool rerank + the ndb retrieval lane). See [catalog §115](.docs/09_feature_catalog/opt_in_features.md). |
| **Jev Structured Reads** (`structured_reads`, in `katgpt-forward`) | Read-only seeded-canvas decision primitive — the vLLM PR #57250 contract on our D2F stack (Issue 859, Research 574): seed a canvas (fixed schema tokens + mask at free slots) → ONE bidirectional denoise step (`forward_bidirectional_positions_into`, the identical call `denoise_loop` makes) → per-free-slot {argmax, EXACT full-marginal logprob over a caller-supplied label-id list, entropy over the subset-NORMALIZED label distribution} → return, **never committing**. Zero new substrate — composition only; `ac_prefix`/`canvas_schema` deliberately not consumed (the D2F forward is natively the right conditioning shape). `sample_label_index` is the T5 stochastic re-read enabler (deterministic re-reads are bit-identical). | Opt-in POC — G1/G1b/G2/G4 PASS (Bench 816): label logprobs full-marginal-exact (f64 cross-check 1e-5), canvas bit-identical after read, read/full-loop median ratio 0.4588 (33 interleaved pairs, Issue-723 discipline), 0 allocs across 32 reads (canary-armed, release+alloc_tracking). G3 154/154. Accuracy axis rides the 4090 reference run (T1, deferred on sibling GPU occupancy). See [catalog §117](.docs/09_feature_catalog/opt_in_features.md). |
...
### 🧠 qsg_gossip — Crowds That Obey Physics (Plan 589, arXiv:2603.24676)

Quantized Simplex Gossip — the minimal statistical mechanics of multi-agent
**belief** interaction (Tanaka, Mar 2026). Where `signed_coupling_dynamics`
owns the crowd's **temperature** axis, QSG owns the **bandwidth + adaptation**
axes on persistent simplex state:

```text
N agents hold beliefs x_i ∈ Δ_{K−1}; each interaction:
  speaker samples a length-m message y ~ Cat(x_S)  (Hard / TopM / Soft)
  listener blends  x_L ← (1−α)·x_L + α·y
```

Quantized communication injects sampling variance even under perfect
neutrality — one agent's arbitrary draw is the next agent's evidence
("memetic drift": consensus can be a lottery). The kernel is TEST-GATED on
the paper's closed-form laws: extra collapse drift `α²/(mN²)·E[1−‖x_S‖²]`
(speaker uncertainty is the fuel, message length m divides it),
`t_cons ≈ (mN²/α²)·log((1−1/K)/(1−U⋆))`, Soft = exact martingale that never
breaks symmetry, α=1 → voter model with a uniform winner, and the
drift-vs-selection crossover `Γh = mNh/α`.

**GOAT G1–G4 ALL PASS** ([Bench 703](.benchmarks/703_qsg_gossip_goat.md)):
21 lib tests pin the one-step identities (Thm 1/2, 1/m corollary, Soft
martingale + contraction, voter reduction); the physics harness reproduces
`U(t)` against the exact two-moment (U,V) flow (ensemble of 48 runs), the N²
consensus scaling (log-log slope 2.00 ± 0.15), the 1/(mN²) early-drift law,
the tempered-sampling `ΓT` crossover, and — the paper's headline — the
single-parameter **collapse** of fixation onto Γh (±0.03 across an 8×
population range), with one honest deviation recorded: the measured logistic
carries effective slope ≈ ½ (σ(Γh/2)), the pair-choice heterogeneity variance
the first-order diffusion drops. **~20.6 ns/interaction** (K=8, N=1024;
10⁶ interactions in 20.6 ms, release), 0 allocs on every hot path,
RNG-free (caller-supplied uniforms — replayable).

The `m` axis is the NpcCommsBus wire budget read as a **dynamical** dial:
halving the per-message budget doubles monoculture drift. External fields
(the selection pole) compose from the exported pieces — tilt a copy of the
speaker row, `qsg_draw_categorical` from the tilted copy,
