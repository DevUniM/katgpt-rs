...
- [ ] T1.6 corpus-distance abstain arm — **SEPARATE TASK, marked UNVALIDATED**: the SalesRLAgent extraction (Research 576 §2.1) comes from a single-author, synthetic-only, no-code paper; it must prove itself against the score-threshold ABSTAIN baseline (selective accuracy at matched abstain rates) before it joins the default arm
