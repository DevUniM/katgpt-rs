...
Status: **IMPLEMENTED (katgpt-rs Phase 1+2, Plan 604, 2026-09-21)** — `bridge_certified` feature in katgpt-core: `BridgeCertified` trait + `MotifDirections` impl, certified kernel (`dot_f32_ordered` + Cephes `fast_sigmoid`), G1 harness + audit + alloc-gate tests. **G1 PASS**: corpus digest bit-identical aarch64 ↔ x86_64/Rosetta (`.benchmarks/604_bridge_certified_g1.md`). T1.4 soft-sigmoid OBSOLETE-BY-SUBSTRATE (fast_sigmoid already IS the fixed-coefficient software sigmoid; the certified path pins it by name — the divergence vector was the platform-SIMD dot, not the sigmoid). Phase 3 (drift sampler riir-ai / slashing + architecture_root pin riir-chain) still NOT implemented; promotion correctly declined (no consumer). The determinism contract ALSO ships BY MIRROR in riir-chain's AOI primitive (Plan 017 G1: ≥10⁴ bit-identity corpus, `tests/aoi_g1_determinism.rs` + `aoi_g1_cross_arch_corpus.rs`; `riir-chain/src/aoi/mod.rs` "BridgeCertified mirror").
