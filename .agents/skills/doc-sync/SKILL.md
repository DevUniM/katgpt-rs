...
| `riir-shader` | **No `.docs/` folder** — AGENTS.md/BOUNDARY.md/HISTORY.md/README-centric (the WebGPU visual-effect substrate as Bevy plugins — ports every vgpu.sh example to wasm32+native as composable plugins; leaf, zero riir-* deps; upstream `vercel-labs/vgpu` @ 42bc4bc, MIT, attribution header per shader). **Row ADDED 2026-09-12** — born 2026-09-12 00:50 (scaffold `464e95a`), censused same-day by a gate-red-driven doc-sync run; the census tables + `repo_set.txt` + katgpt-rs AGENTS §Repo count had all stayed at 18 until the gates caught it. | yes (README) | `.issues` 1 · `.plans` 1 | `develop` |
...
folder to any repo that already has numbered folders — measured 2026-09-12 as
**11 of the 19**: `katgpt-rs`, `riir-ai`, `riir-chain`, `riir-clippy`,
`riir-dapps`, `riir-deployer`, `riir-game-sdk`, `riir-neuron-db`, `riir-train`,
`riir-viewbridge`, `seal-remake`. **Or** that gives a `.docs/` to one of the
six with none (`katgpt-web`, `riir-dao`, `riir-esp32`,
`riir-kat`, `riir-mmorpg-examples`, `riir-shader`) or the one whose `.docs/` holds no document
